...
pass_config = click.make_pass_decorator(Config, ensure=True) #ensure = True means object does not exist, it will be created and passed to the command.
...
def read_config_yaml():
    ...
    try:
        home = get_kanbancli_home()
        with open(home + "/.kanbancli.yaml", 'r') as stream:
            try:
                return yaml.safe_load(stream)
...
def get_kanbancli_home():
    home = os.environ.get('KANBANCLI_HOME')
    if not home:
